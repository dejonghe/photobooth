...
class BoothPrint(object):
    ...
    def _combine(self, images):
        ...
        text_width, text_height = draw.textsize(self.title_message, font)
        logger.debug("Text Width: {}".format(text_width))
        logger.debug("Text Height:{}".format(text_height))
        position = ((self.title_border[0]-text_width)/2,(self.print_size[1] - (text_height * 1.5)))
        #draw.text((200, self.print_size[1] - self.title_border[1] + 40),self.title_message,'#fff',font=font)
        draw.text(position,self.title_message,'#fff',font=font)
